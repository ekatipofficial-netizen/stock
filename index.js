...
// เพิ่มแถวใหม่ลงชีต Movement Log ทุกแถวที่อ่านได้วันนี้ (ใส่วันที่กำกับให้อัตโนมัติ)
...
async function updateMasterCurrentStock(items) {
  ...
  for (const item of items) {
    ...
    if (rowNum) {
      updates.push({ range: `Master!G${rowNum}`, values: [[item['คงเหลือ']]] });
    } else {
      unmatched.push(name);
    }
  }
...
app.post('/webhook', async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      replyMessage = 'อ่านได้ผลลัพธ์นี้ (บันทึกลง Sheet ไม่สำเร็จ เช็คด้วยตนเอง):\n' + resultJsonText + '\n\nข้อผิดพลาด: ' + e.message;
    }
