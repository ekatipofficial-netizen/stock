...
// สร้าง Flex Message การ์ดเดียว รวมทั้งผลตรวจสอบสต็อกวันนี้ และรายการที่ต้องสั่งเพิ่ม (แยกตามซัพพลายเออร์)
function buildValidationFlex(items, saveResult = {}, summaryRows = []) {
  ...
  // แบ่งเป็นหมวดตาม STK (เรียงแล้วจากขั้นตอนก่อนหน้า)

  // จัดกลุ่มเป็นหมวดตาม STK เรียงลำดับ ภายในบับเบิลเดียว (ไม่แบ่งเป็นหลายการ์ดอีกต่อไป)
  const groups = [];
  let current = null;
  for (const item of sorted) {
    if (!current || current.stk !== item['stk']) {
      current = { stk: item['stk'], items: [] };
      groups.push(current);
    }
    current.items.push(item);
  }

  const sections = groups.flatMap((group, groupIdx) => {
    const header = {
      type: 'box',
      layout: 'vertical',
      margin: groupIdx === 0 ? 'none' : 'lg',
      contents: [
        { type: 'text', text: `📋 ${group.stk || 'ไม่ทราบฟอร์ม'} (${group.items.length} รายการ)`, weight: 'bold', size: 'sm', color: '#888888' },
  ...
  // ส่วนสรุปสั่งซื้อ ต่อท้ายในการ์ดเดียวกัน จัดกลุ่มตามซัพพลายเออร์
  const orderGroups = {};
  ...
  const orderSections = Object.entries(orderGroups).flatMap(([supplier, orderItems], idx) => {
    ...
    const itemBoxes = orderItems.map(row => ({
      ...
      contents: [
        ...
        { type: 'text', text: `สั่งเพิ่ม ${row[6]} ${row[2] || ''}`, size: 'sm', weight: 'bold', color: '#FF0000' }
      ]
    }));
    return [header, ...itemBoxes];
  });
  const orderBlock = [
    ...
    {
      type: 'text',
      text: summaryRows.length > 0 ? `📦 ต้องสั่งเพิ่ม (${summaryRows.length} รายการ)` : '📦 สต็อกเพียงพอ ไม่ต้องสั่งเพิ่ม 👍',
      weight: 'bold',
      size: 'md',
      margin: 'lg'
    },
    ...(summaryRows.length > 0 ? [{ type: 'separator', margin: 'xs' }, ...orderSections] : [])
  ];
  ...
  return {
    ...
    contents: {
      ...
      body: {
        ...
        spacing: 'sm',
        contents: [
          { type: 'text', text: '✅ ผลตรวจสอบสต็อกวันนี้', weight: 'bold', size: 'lg' },
          { type: 'text', text: altText, size: 'xs', color: '#888888', wrap: true },
