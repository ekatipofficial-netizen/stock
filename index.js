...
// การ์ดที่ 1-7: ผลตรวจสอบแยกตาม STK01-STK07 ตามลำดับ / การ์ดสุดท้าย: สรุปสั่งซื้อแยกตามซัพพลายเออร์
function buildValidationFlex(items, saveResult = {}, summaryRows = []) {
  ...
  // จัดกลุ่มเป็นรายการต่อ STK หนึ่งกลุ่ม = หนึ่งการ์ด
  const groups = [];
  let current = null;
  ...
  const stkBubbles = groups.map(group => {
    ...
    return {
      ...
      body: {
        ...
        spacing: 'sm',
        contents: [
          { type: 'text', text: `📋 ${group.stk || 'ไม่ทราบฟอร์ม'}`, weight: 'bold', size: 'lg' },
  ...
  const parts = [`อ่านได้ ${items.length} รายการ จาก ${groups.length} ฟอร์ม`];
