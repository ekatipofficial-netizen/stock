...
// สร้าง Flex Message สรุปสั่งซื้อ แยกเป็นการ์ดตามซัพพลายเออร์
function buildSupplierSummaryFlex(rows) {
  if (rows.length === 0) {
    return { type: 'text', text: `📦 สรุปสต็อกวันนี้ (${new Date().toLocaleDateString('th-TH')})\n\nสต็อกทุกอย่างเพียงพอ ไม่มีรายการที่ต้องสั่งเพิ่มครับ 👍` };
  }
  ...
  const bubbles = Object.entries(groups).slice(0, 12).map(([supplier, items]) => ({
    type: 'bubble',
    size: 'giga',
    body: {
      type: 'box',
      ...
      spacing: 'sm',
      contents: [
        { type: 'text', text: supplier, weight: 'bold', size: 'md', color: '#1E7FD9', wrap: true },
        { type: 'text', text: `${items.length} รายการที่ต้องสั่ง`, size: 'xs', color: '#888888' },
        { type: 'separator', margin: 'sm' },
        ...items.map(row => ({
          type: 'box',
          layout: 'vertical',
...
app.post('/webhook', async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      const validationCard = buildValidationFlex(checkedItems, { updatedCount, unmatched });
      replyMessage = [validationCard];

      // แนบการ์ดสรุปสั่งซื้อต่อท้ายทันที (ไม่ต้องรอรอบเช้า) ถ้าดึงไม่สำเร็จก็ข้ามไป ไม่ให้กระทบการ์ดหลัก
      try {
        const summaryRows = await fetchSummaryRows();
        replyMessage.push(buildSupplierSummaryFlex(summaryRows));
      } catch (e) {
        console.warn('⚠️ ดึงสรุปสั่งซื้อไม่สำเร็จ (ข้ามไป):', e.message);
      }
    } catch (e) {
